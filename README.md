...
To build a data model to support analysis of stock market and real estate data. Specifically, to see what correlations, if any, can be drawn between specific stock performances and real estate performance. I forsee this data being useful for contructing analytical models, reports, and tables for drawing conclusions regarding this topic.
...
### Companies
Finally, I've tied in Company data for the Stock/ETF data so that it is easier to draw correlations between types of company's stock trends and see what conclusions could be drawn.
...
1. Have a Airflow instance, AWS Redshift cluster, and AWS key and secret ready to be utilized.
2. Copy the dag and plugin direcotries to you airflow directory
3. Uncompress the Data files (they are compressed into 3 parts, WinRAR will easily handle the uncompression) to your HOME directory (should be $HOME/Data/Companies, $HOME/Data/Homes, $HOME/Data/Stocks, $HOME/Data/ETFs when uncompressed)
4. In the Airflow WebUI, setup the following connections: 
...
5. Turn on the stock\_home\_dag DAG.
6. Track it's progress and use the data quality task logs to verify that the tables and temp directories ar ebeing populated properly.
